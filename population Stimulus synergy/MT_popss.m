...
nCells_vec = 2.^(4:9); %2^10;

%nCells_vec = 128;
%nReps_vec = [20*ones(1,length(find(nCells_vec<=1024))) 10*ones(1,length(find(nCells_vec>1024)))];
nReps_vec = [40*ones(1,length(find(nCells_vec<=1024))) 10*ones(1,length(find(nCells_vec>1024)))];
%nReps_vec = 50*ones(size(nCells_vec));
% nReps_vec = [40*ones(1,length(find(nCells_vec<512))) 20*ones(1,length(find(nCells_vec>=512)))];
...
for nidx = 1:length(nCells_vec)
    ...
    for tidx = 1:length(Twin_vec)
       ...
       Twin = Twin_vec(tidx);
%         
%         ENTs = -sum(psvals.*log2(psvals)); %
%         ENTd = -sum(pdvals.*log2(pdvals));
%         
...
%                 PV.est.bias(:,s,d) = PV.est.mean(:,s,d)-[svals(s);dvals(d)];  computed above.
...
%make pop figure for Synergy paper

%MI encoded about direction, speed, and vector in population relative to
%maxMI defined by the precision of pursuit

%MI/MImax vs log2 N at 100ms LOLE (saturates at 128 neurons)
figure
tidx=5;
yvals = MI.LOLE(tidx,:)./MImax(1,1);
ebars = MI_std.LOLE(tidx,:)./MImax(1,1);
shadedErrorBar(log2(nCells_vec),yvals,ebars,'lineProps','ko-','transparent',1);
hold on
xvals = 0:0.1:10;
plot(xvals,ones(1,length(xvals)),'k--');
set(gca,'Box','Off','TickDir','Out');
set(gca,'FontSize',20,'TickLength',[.025 , .01]);
%ylim([0 1.5]);
xlim([min(log2(nCells_vec)) max(log2(nCells_vec))]);
xlabel('log2 N');
ylabel('MI/MIpurs');

%frac syn vs log2 N for LOLE at 100ms

figure;
tidx=5;
%yvals = (MI.LOLE(tidx,:)-MI.Marg(tidx,:))./MI.Marg(tidx,:);
yvals = FracSyn.LOLEMarg(tidx,:);
%approx std of frac synergy
%vals =  0.5*(MI_std.LOLE(tidx,:).^2 + MI_std.Marg(tidx,:).^2);
%ebars =  sqrt(vals./MI.Marg(tidx,:));
ebars = FracSyn_std.LOLEMarg(tidx,:);
shadedErrorBar(log2(nCells_vec),yvals,ebars,'lineProps','ko-','transparent',1);
hold on
xvals = 0:0.1:10;
plot(xvals,0.2*ones(1,length(xvals)),'k--');
set(gca,'Box','Off','TickDir','Out');
set(gca,'FontSize',20,'TickLength',[.025 , .01]);
ylim([0 .3]);
xlim([min(log2(nCells_vec)) max(log2(nCells_vec))]);
...
%fracsyn vs time for N=128
figure;
nidx = find(nCells_vec==128);
tvals = 1000*Twin_vec;
yvals = FracSyn.LOLEMarg(:,nidx);
ebars = FracSyn_std.LOLEMarg(:,nidx);
%approx std of frac synergy
% vals =  0.5*(MI_std.LOLE(tidx,:).^2 + MI_std.Marg(tidx,:).^2);
% ebars =  sqrt(vals./MI.Marg(tidx,:));
shadedErrorBar(tvals,yvals,ebars,'lineProps','ko-','transparent',1);
hold on
plot([0 tvals],0.2*ones(1,length(tvals)+1),'k--');
set(gca,'Box','Off','TickDir','Out');
set(gca,'FontSize',20,'TickLength',[.025 , .01]);
ylim([0 .4]);
xlabel('time (ms)');
