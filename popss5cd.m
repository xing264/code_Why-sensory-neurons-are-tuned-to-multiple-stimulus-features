...
% plot errors  Errd and Errs
figure;
tvals = 1000*Twin_vec;

yvals_mean = [sqrt(degVard.GL(:,nidx))'; sqrt(degVard.PV(:,nidx))'; sqrt(degVard.NB(:,nidx))'; sqrt(degVard.NBjoint(:,nidx))'; sqrt(degVard.Marg(:,nidx))'; sqrt(degVard.LOLE(:,nidx))'];
yvals_std = [sqrt(degVard_std.GL(:,nidx))'; sqrt(degVard_std.PV(:,nidx))'; sqrt(degVard_std.NB(:,nidx))'; sqrt(degVard_std.NBjoint(:,nidx))'; sqrt(degVard_std.Marg(:,nidx))'; sqrt(degVard_std.LOLE(:,nidx))'];
hold all;
for idx = 1:6
%     if idx==2 
%         plot(tvals, yvals_mean(idx,:),'o-');
%     else
    shadedErrorBar(tvals,yvals_mean(idx,:),yvals_std(idx,:),'lineProps','o-','transparent',1);
%     end
    hold on
end
set(gca,'Box','Off','TickDir','Out','FontSize',20,'TickLength',[.025 , .01]);
clear idx
plot([0 tvals],ones(1,length(tvals)+1)*3,'k--','LineWidth',2);
set(gca,'FontSize',18,'Box','Off','TickDir','Out');
legend('GL','PV','NB (1D)','NB (2D)','ML (1D)','ML (2D)','pursuit');
xlabel('time (ms)');
ylabel('RMSE direction (deg)');
axis([0 200 0 30]);


figure;
tvals = 1000*Twin_vec;
yvals_mean = [sqrt(FracVars.GL(:,nidx))'; sqrt(FracVars.PV(:,nidx))'; sqrt(FracVars.NB(:,nidx))'; sqrt(FracVars.NBjoint(:,nidx))'; sqrt(FracVars.Marg(:,nidx))'; sqrt(FracVars.LOLE(:,nidx))'];
yvals_std = [sqrt(FracVars_std.GL(:,nidx))'; sqrt(FracVars_std.PV(:,nidx))'; sqrt(FracVars_std.NB(:,nidx))'; sqrt(FracVars_std.NBjoint(:,nidx))'; sqrt(FracVars_std.Marg(:,nidx))'; sqrt(FracVars_std.LOLE(:,nidx))'];
for idx = 1:6
    if idx==2 
       plot(tvals, yvals_mean(idx,:),'-o');
    else
...
clear idx
plot([0 tvals],ones(1,length(tvals)+1)*0.1,'k--','LineWidth',2);
set(gca,'Box','Off','TickDir','Out','FontSize',20,'TickLength',[.025 , .01]);
legend('GL','PV','NB (1D)','NB (2D)','ML (1D)','ML (2D)','pursuit');
xlabel('time (ms)');
ylabel('RMSE log2 speed');
axis([0 200 0 0.7]);
